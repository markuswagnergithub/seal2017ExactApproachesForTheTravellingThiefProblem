...
public class DpEnhancedTest {


	private static final double delta = 1e-10;

	@Test
	public void testDpTest1n4m6() throws FileNotFoundException{
		...
		DpEnhanced dp = new DpEnhanced(ttp, stops);
		...
		Assert.assertEquals(Util.bitSet(new int[] {6, 7}), dp.getSolutionBitSet());
		
		Assert.assertEquals(6, dp.getTotalWeight());
		Assert.assertEquals(15, dp.getTotalProfit());
		Assert.assertEquals(-72.19771665801764, dp.getMaxBenefit(), delta);
		Assert.assertEquals(-72.19771665801764, dp.Beta(), delta);
	...
	public void testDpTest1n8m14() throws FileNotFoundException{
		...
		Assert.assertEquals(Util.bitSet(new int[] {1, 3, 5, 9, 11, 13}), dp.getSolutionBitSet());
		
		Assert.assertEquals(167, dp.getTotalWeight());
		Assert.assertEquals(395, dp.getTotalProfit());
		Assert.assertEquals(-113.34139844423362, dp.getMaxBenefit(), delta);
		Assert.assertEquals(-113.34139844423362, dp.Beta(), delta);
	...
	public void testDpEil51N50Uncorr01() throws FileNotFoundException{
		...
		Ttp ttp = TtpLoader.load("experiments/eil51/" + instance);
	...
	public void testDpEil51N50Corr06() throws FileNotFoundException{
		...
		System.out.println("tour: " + Arrays.toString(stops));
		
		LookupTable table = new LookupTable(ttp, stops);
		
		NavigableMap<Long, PackingPlanExtra> map = table.baseTable.get(23);
		Util.printMap(map);
		...
		System.out.println("true benefit:" + ttp.benefit(stops, dp.getSolutionBitSet()));

//		Assert.assertEquals(21446, dp.getTotalWeight());
//		Assert.assertEquals(27346, dp.getTotalProfit());
		Assert.assertEquals(Util.bitSet("11110000110001010111100100000011000111101011111111"), dp.getSolutionBitSet());
	...
	public void testDpEil76N75Uncorr01() throws FileNotFoundException{
		...
		Ttp ttp = TtpLoader.load("experiments/eil76/" + instance);
		int[] stops = TourLoader.load(tourfile, TourType.lkh);
	...
	public void testDpEil101N1000UncorrSw10() throws FileNotFoundException{
		...
		String tourfile = "experiments/eil101/eil101.linkern.tour";
		
		Ttp ttp = TtpLoader.load("experiments/eil101/" + instance);
		int[] stops = TourLoader.load(tourfile, TourType.lkh);
